...
public class HDFSTotalFileNumberReportThread extends HDFSReportThread {
    ...
    public void work() {
        try {
            ...
            message.setCompKey(activeNNIP + Constants.TRANSFER_VERTICAL_DELIMITER + HDFS_SERVER_KEY + HDFSUtils.nnHTTPPort(activeNNHostname));
            message.setHostIP(activeNNIP);
            message.setMetricCode(HDFS_STATUS_CODE);
            message.setMetricType(MetricType.STATUS);
            message.setMetricValue(Long.toString(HDFSUtils.totalFilesNum()));
